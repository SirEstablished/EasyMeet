...
CREATE INDEX IF NOT EXISTS idx_escrow_customer ON public.escrow_orders(customer_id);
CREATE INDEX IF NOT EXISTS idx_escrow_pro ON public.escrow_orders(professional_id);
CREATE INDEX IF NOT EXISTS idx_escrow_convo ON public.escrow_orders(conversation_id);
...
DROP POLICY IF EXISTS "escrow parties read" ON public.escrow_orders;
CREATE POLICY "escrow parties read" ON public.escrow_orders
  FOR SELECT TO authenticated
  USING (customer_id = auth.uid() OR professional_id = auth.uid() OR public.has_role(auth.uid(),'admin'));
DROP POLICY IF EXISTS "escrow customer insert" ON public.escrow_orders;
CREATE POLICY "escrow customer insert" ON public.escrow_orders
  FOR INSERT TO authenticated
  WITH CHECK (customer_id = auth.uid());
DROP POLICY IF EXISTS "escrow parties update" ON public.escrow_orders;
CREATE POLICY "escrow parties update" ON public.escrow_orders
  FOR UPDATE TO authenticated
  USING (customer_id = auth.uid() OR professional_id = auth.uid() OR public.has_role(auth.uid(),'admin'))
...
CREATE POLICY "dispute parties read" ON public.escrow_disputes
  ...
  USING (
    ...
    OR EXISTS (SELECT 1 FROM public.escrow_orders o
...
CREATE POLICY "dispute parties insert" ON public.escrow_disputes
  ...
  WITH CHECK (
    opened_by = auth.uid()
    AND EXISTS (SELECT 1 FROM public.escrow_orders o
                WHERE o.id = order_id AND (o.customer_id = auth.uid() OR o.professional_id = auth.uid()))
  );
...
CREATE POLICY "evidence parties read" ON public.escrow_dispute_evidence
  ...
  USING (
    ...
    OR EXISTS (
      ...
      JOIN public.escrow_orders o ON o.id = d.order_id
