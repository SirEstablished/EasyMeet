-- RPC: atomically create an order + escrow row for a paid service agreement
-- and notify the provider. Used by the escrow payment flow on the client.
...
BEGIN
  ...
  v_commission := round(p_amount * 0.03, 2);
  v_payout := round(p_amount - v_commission, 2);

  INSERT INTO public.orders (
    customer_id, provider_id, product_id, service_id, kind,
    service_title, amount, commission_amount, currency, notes,
    payment_ref, payment_status, status
  ) VALUES (
    p_customer_id, p_provider_id, NULL, NULL, 'service',
    v_agreement.job_title, p_amount, v_commission, 'NGN', v_agreement.job_description,
    p_payment_ref, 'paid', 'pending'
  ...
  INSERT INTO public.escrow (
    ...
    v_agreement.job_title, p_amount, v_commission, v_payout, 'holding',
    p_payment_ref, p_payment_ref, now()
  ) RETURNING * INTO v_escrow;

  BEGIN
    INSERT INTO public.notifications (
      user_id, recipient_id, sender_id, type, title, message, body, read
    ) VALUES (
